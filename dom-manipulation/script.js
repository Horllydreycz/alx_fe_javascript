...
function createAddQuoteForm() {
  ...
  quoteContainer.parentNode.insertBefore(
    formContainer,
    quoteContainer.nextSibling
  );
}
...
function showRandomQuote() {
  ...
  if (filteredQuotes.length > 0) {
    ...
    document.getElementById(
      "quoteDisplay"
    ).innerHTML = `<p>"${quote.text}"</p><p><em>- ${quote.category}</em></p>`;
    ...
    serverQuotes.forEach((serverQuote) => {
      const existingQuote = quotes.find(
        (localQuote) =>
          ...
          localQuote.serverId === serverQuote.serverId
      );
...
function handleConflicts(conflicts) {
  ...
  showSyncNotification(
    `Resolved ${conflicts.length} conflicts (server version kept)`
  );
}
...
async function retryFailedSyncs() {
  ...
  const remainingFailures = failedSyncs.filter(
  ...
  showSyncNotification(
    `Retry completed: ${
      retryResults.filter((r) => r.success).length
    } successful`
  );
}
